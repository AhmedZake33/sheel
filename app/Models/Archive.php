...
//            Storage::disk('local')->put($archive->calculatePath(), $content);
    ...
    {
        if (empty($this->id) || empty($this->parent_id)) {
            return;
        }
    ...
    public function isOwner($user = null)
    {
        ...
        while ($archive->id != 0) {
            ...
            if ($user->archive_id == $archive->id) return true;   // personal photo
    ...
    public function download()
    {
        ...
        if(in_array($this->extension,['jpg','png','gif','tif','jpeg','bmp'])){
           return response($file, 200, [
            ...
            return Storage::download($this->calculatePath());
            // return Storage::download($this->calculatePath() , $this->title .'.' . $this->extension);
        }
    }
...
//        }
    ...
    {
        return encryptData("$this->id, $this->updated_at", env("WEBSITE_SHARED_KEY", NULL));
    }
    ...
    //     // check if api come from website or admin
    //     if(strstr($referer , 'https://naqaae.eg') || strstr($referer , 'http://naqaae.eg')){
    //         if (strstr($path, "/Website/") || strstr($path, "الموقع")) {
    //             return true;
    //         }    
    //     }
    ...
    public function getSizes()
    {
        ...
        foreach($ids as $id){
            ...
            if($archive)
            {
                if($archive->isFile())
                {
                    $size = $size + $archive->calculateSize();
                }
            }
        }
        return $size;
